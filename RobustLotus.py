...
# -*- coding: utf-8 -*-
"""
LotusMesh → OBJ / CityJSON Pipeline
=====================================
Single-script pipeline that merges:
  1. LotusMesh  – point-cloud → voxel → shell → decimate → generalise
  2. obj2cityjson – OBJ mesh → CityJSON (LOD2 solid with semantics)

A Tkinter GUI collects all parameters before any heavy work is done.
...
class LotusMesh:
    ...
    log_fn            : callable(str) – sink for progress messages
    """
...
# ══════════════════════════════════════════════════════════════════════════════
#  SECTION 3 ─ OBJ → CityJSON CONVERTER
# ══════════════════════════════════════════════════════════════════════════════

def _load_obj_direct(filepath):
    """Read v and f tags from an OBJ file."""
    verts, faces = [], []
...
def _reclassify_surrounded_by_roof(faces, semantics_list, log_fn=print):
    ...
    log_fn(f"   [Neighbour] {floor_chg} OuterFloor + {ground_chg} GroundSurface → WallSurface.")
...
def obj_to_cityjson(obj_path, output_path, epsg_code="7415", log_fn=print):
    ...
    output_path : str  – output .city.json file
    epsg_code   : str  – EPSG code for the CRS (default '7415')
    log_fn      : callable(str)
    """
...
class PipelineGUI(tk.Tk):
    """
    Tkinter front-end for the LotusMesh → OBJ / CityJSON pipeline.
    """

    # ── colours / fonts (subtle dark theme) ──────────────────────────────────
    BG        = "#1e1e2e"
    FG        = "#cdd6f4"
    ACCENT    = "#89b4fa"
    ENTRY_BG  = "#313244"
    ...
    def _build_ui(self):
        ...
        # ── Header ────────────────────────────────────────────────────────────
        hdr = tk.Label(self, text="  LotusMesh → OBJ / CityJSON Pipeline",
        ...
        # ── Input file ────────────────────────────────────────────────────────
        self._section("Input Point Cloud (.ply)")
        row = tk.Frame(self, bg=self.BG)
        row.pack(fill="x", **pad)
        self._ply_var = tk.StringVar()
        tk.Entry(row, textvariable=self._ply_var, bg=self.ENTRY_BG,
                 fg=self.FG, font=self.FONT, insertbackground=self.FG,
                 relief="flat").pack(side="left", fill="x", expand=True)
        tk.Button(row, text="Browse…", command=self._browse_ply,
        ...
        # EPSG row (shown only for CityJSON)
    ...
    def _param_row(self, parent, row, label, var, from_, to, increment):
        ...
        sb.grid(row=row, column=1, sticky="w", padx=(8, 24), pady=3)
    ...
    def _on_fmt_change(self):
        ...
        # Re-derive the output path extension whenever the format changes
        ply = self._ply_var.get().strip()
        if ply:
            self._auto_output_path(ply)
    ...
    def _browse_out(self):
        ...
        if fmt == "cityjson":
            ft = [("CityJSON", "*.city.json *.json"), ("All files", "*.*")]
            defext = ".city.json"
        else:
            ft = [("OBJ mesh", "*.obj"), ("All files", "*.*")]
            defext = ".obj"
        p = filedialog.asksaveasfilename(
            title="Save output as…",
            filetypes=ft, defaultextension=defext)
        if p:
            self._out_var.set(p)

    def _auto_output_path(self, ply_path):
        """Suggest an output path next to the input file."""
        base = os.path.splitext(ply_path)[0]
        fmt  = self._fmt_var.get()
        ext  = ".city.json" if fmt == "cityjson" else ".obj"
        self._out_var.set(base + ext)

    # ── Logging ───────────────────────────────────────────────────────────────

    def _log(self, msg):
        """Thread-safe log helper: push to queue, picked up by _poll_log."""
    ...
    def _run(self):
        ply_path = self._ply_var.get().strip()
        out_path = self._out_var.get().strip()

        if not ply_path:
            messagebox.showerror("Missing input", "Please select an input PLY file.")
            return
        ...
        t = threading.Thread(
            ...
            args=(ply_path, out_path, fmt, epsg,
                  voxel_size, decimate_pc, smooth_iter,
                  taubin_lambda, taubin_mu),
            daemon=True)
        t.start()

    def _pipeline_thread(self, ply_path, out_path, fmt, epsg,
                         voxel_size, decimate_pc, smooth_iter,
                         taubin_lambda, taubin_mu):
        try:
            self._log("=" * 50)
            self._log("PIPELINE STARTED")
            self._log(f"  Input      : {ply_path}")
            ...
            if fmt == "obj":
                ...
                # Strip compound extensions like ".city.json" before adding "_mesh.obj"
                base = out_path
            ...
            # Always pass file_type explicitly – trimesh must not guess from
            # the extension (e.g. ".city.json" would trigger a missing exporter).
            final_mesh.export(obj_path, file_type="obj")

            # ── Stage 3: CityJSON (optional) ──────────────────────────────────
            if fmt == "cityjson":
            ...
            # Show completion popup from main thread
